...
NEMOCLAW_ENDPOINT_URL=https://integrate.api.nvidia.com/v1 # or http://172.17.0.1:18080/v1 if using tls proxy
...
### Host-side TLS proxy (optional)

Some hosts resolve the NVIDIA inference endpoint to `127.0.0.1` locally (for example
when a corporate VPN or split-horizon DNS intercepts the hostname), which means the
Docker sandbox reaches a different address than the host. In that case, run
`scripts/host-tls-proxy.py` on the host as a plain-HTTP forwarder: the sandbox
connects to it over plain HTTP, and the proxy handles TLS toward the upstream on
the host's behalf.

Set `NEMOCLAW_ENDPOINT_URL=http://172.17.0.1:18080/v1` in `.env`, then start
the proxy before onboarding:

```bash
mkdir -p .tmp
setsid -f scripts/host-tls-proxy.py \
  --upstream "${NEMOCLAW_INFERENCE_UPSTREAM:-https://your-inference-host}" \
  --listen 0.0.0.0 \
  --port 18080 \
  > .tmp/host-tls-proxy.log 2>&1 < /dev/null
```

Keep the proxy running while the sandbox is running. If you do not need it,
leave `NEMOCLAW_ENDPOINT_URL` pointing directly at the HTTPS inference endpoint
and skip this step.
