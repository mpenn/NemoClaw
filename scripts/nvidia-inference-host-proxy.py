#!/usr/bin/env python3
"""Small HTTP reverse proxy for the NVIDIA local TLS inference endpoint.

This is intended for local OpenShell gateways that cannot validate or cleanly
read from the host's mkcert-backed https://inference-api.nvidia.com endpoint.
The proxy listens on plain HTTP on the host and forwards requests to the real
HTTPS endpoint using the host trust store.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Upstream origin, without /v1",
    )
