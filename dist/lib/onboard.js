...
const { getDefaultOllamaModel, getBootstrapOllamaModelOptions, getLocalProviderBaseUrl, getLocalProviderValidationBaseUrl, getOllamaModelOptions, getOllamaWarmupCommand, validateOllamaModel, validateLocalProvider, } = require("./local-inference");
const { DEFAULT_CLOUD_MODEL, getProviderSelectionConfig, parseGatewayInference, } = require("./inference-config");
const { inferContainerRuntime, isWsl, shouldPatchCoredns } = require("./platform");
const { resolveOpenshell } = require("./resolve-openshell");
const { prompt, ensureApiKey, getCredential, normalizeCredentialValue, saveCredential, } = require("./credentials");
...
const { assessHost, checkPortAvailable, ensureSwap, getMemoryInfo, planHostRemediation, } = require("./preflight");
...
function detectContainerHostAddress() {
    ...
    try {
        const gateway = runCapture(["docker", "network", "inspect", "bridge",
            "--format", "{{range .IPAM.Config}}{{.Gateway}}{{end}}"], { ignoreError: true }).trim();
...
const { isSandboxReady, hasStaleGateway, isSelectedGateway, isGatewayHealthy, getGatewayReuseState, getSandboxStateFromOutputs, } = gatewayState;
/**
...
function streamGatewayStart(command, env = process.env) {
    ...
    function setPhase(nextPhase) {
        ...
        const phaseLine = nextPhase === "install"
    ...
    function classifyLine(line) {
        if (/ApplyJob|helm-install-openshell|Applying HelmChart/i.test(line))
            return "install";
    ...
    const heartbeatTimer = setInterval(() => {
        ...
        if (bucket === lastHeartbeatBucket)
            return;
        if (Date.now() - lastOutputAt < 3000 && elapsed < 10)
            return;
        const heartbeatLine = currentPhase === "install"
...
function pullAndResolveBaseImageDigest() {
    ...
    let inspectOutput;
    try {
        inspectOutput = runCapture(["docker", "inspect", "--format", "{{json .RepoDigests}}", imageWithTag], { ignoreError: false });
    }
    catch {
        return null;
    }
...
const { compactText, normalizeProviderBaseUrl, isLoopbackHostname, formatEnvAssignment, parsePolicyPresetEnv, } = urlUtils;
...
const { classifyValidationFailure, classifyApplyFailure, classifySandboxCreateFailure, validateNvidiaApiKeyValue, isSafeModelId, isNvcfFunctionNotFoundForAccount, nvcfFunctionNotFoundMessage, shouldSkipResponsesProbe, shouldForceCompletionsApi, } = validation;
...
async function promptValidationRecovery(label, recovery, credentialEnv = null, helpUrl = null) {
    ...
    if (recovery.kind === "credential" && credentialEnv) {
        ...
        const choice = (await prompt("  Options: retry (re-enter key), back (change provider), exit [retry]: ", {
            secret: true,
        }))
            .trim()
            .toLowerCase();
        ...
        const looksLikeToken = API_KEY_PREFIXES.some((p) => choice.startsWith(p)) ||
...
function upsertProvider(name, type, credentialEnv, baseUrl, env = {}) {
    ...
    if (result.status !== 0) {
        const output = compactText(redact(`${result.stderr || ""}`)) ||
...
async function configurePhoenixCollector(agent) {
    ...
    const answer = await prompt(`  Arize Phoenix OTLP endpoint (optional, e.g. http://phoenix-host:4318, leave blank to skip) [${existing || "none"}]: `);
...
async function applyPhoenixEgressPolicy(sandboxName) {
    const phoenixEndpoint = getCredential(PHOENIX_COLLECTOR_ENDPOINT_ENV) ||
 ...
 */
async function configureOutlookTokenManager(agent, hasOutlook) {
    if (!agent || agent.name !== "hermes" || !hasOutlook)
        return null;
    if (isNonInteractive()) {
        ...
        if (envSessionId)
            saveCredential("OUTLOOK_SESSION_UUID", envSessionId);
    ...
    const answer = await prompt(`  Outlook token manager host [${existing}]: `);
    const host = answer.trim() || existing;
    saveCredential(TOKEN_MANAGER_HOST_ENV, host);
    process.env[TOKEN_MANAGER_HOST_ENV] = host;
    console.log(`  ✓ Outlook token manager host: ${host}`);
    const clientId = getCredential("OUTLOOK_CLIENT_ID") || process.env.OUTLOOK_CLIENT_ID || "";
    const tenantId = getCredential("OUTLOOK_TENANT_ID") || process.env.OUTLOOK_TENANT_ID || "";
    ...
    if (!startResult.ok) {
        throw new Error(`Outlook authentication required but token manager is unreachable at ${tmBase}.\n` +
...
async function applyOutlookTokenManagerEgressPolicy(sandboxName) {
    const tokenManagerHost = getCredential(TOKEN_MANAGER_HOST_ENV) || process.env[TOKEN_MANAGER_HOST_ENV];
    if (!tokenManagerHost)
        return;
    const presetEntries = [
...
function patchStagedDockerfile(dockerfilePath, model, chatUiUrl, buildId = String(Date.now()), provider = null, preferredInferenceApi = null, webSearchConfig = null, messagingChannels = [], messagingAllowedIds = {}, discordGuilds = {}, baseImageRef = null) {
    const { providerKey, primaryModelRef, inferenceBaseUrl, inferenceApi, inferenceCompat } = getSandboxInferenceConfig(model, provider, preferredInferenceApi);
    ...
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_PRIMARY_MODEL_REF=.*$/m, `ARG NEMOCLAW_PRIMARY_MODEL_REF=${primaryModelRef}`);
    dockerfile = dockerfile.replace(/^ARG CHAT_UI_URL=.*$/m, `ARG CHAT_UI_URL=${chatUiUrl}`);
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_INFERENCE_BASE_URL=.*$/m, `ARG NEMOCLAW_INFERENCE_BASE_URL=${inferenceBaseUrl}`);
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_INFERENCE_API=.*$/m, `ARG NEMOCLAW_INFERENCE_API=${inferenceApi}`);
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_INFERENCE_COMPAT_B64=.*$/m, `ARG NEMOCLAW_INFERENCE_COMPAT_B64=${encodeDockerJsonArg(inferenceCompat)}`);
    ...
    if (proxyHostEnv && PROXY_HOST_RE.test(proxyHostEnv)) {
        dockerfile = dockerfile.replace(/^ARG NEMOCLAW_PROXY_HOST=.*$/m, `ARG NEMOCLAW_PROXY_HOST=${proxyHostEnv}`);
    }
    const proxyPortEnv = process.env.NEMOCLAW_PROXY_PORT;
    if (proxyPortEnv && PROXY_PORT_RE.test(proxyPortEnv)) {
        dockerfile = dockerfile.replace(/^ARG NEMOCLAW_PROXY_PORT=.*$/m, `ARG NEMOCLAW_PROXY_PORT=${proxyPortEnv}`);
    }
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_WEB_SEARCH_ENABLED=.*$/m, `ARG NEMOCLAW_WEB_SEARCH_ENABLED=${webSearchConfig ? "1" : "0"}`);
    ...
    dockerfile = dockerfile.replace(/^ARG NEMOCLAW_DISABLE_DEVICE_AUTH=.*$/m, `ARG NEMOCLAW_DISABLE_DEVICE_AUTH=1`);
    if (messagingChannels.length > 0) {
        dockerfile = dockerfile.replace(/^ARG NEMOCLAW_MESSAGING_CHANNELS_B64=.*$/m, `ARG NEMOCLAW_MESSAGING_CHANNELS_B64=${encodeDockerJsonArg(messagingChannels)}`);
    }
    ...
    for (const envName of sourceEtlBuildArgs) {
        ...
        if (!envValue)
            continue;
        dockerfile = dockerfile.replace(new RegExp(`^ARG ${envName}=.*$`, "m"), `ARG ${envName}=${envValue}`);
    }
    if (Object.keys(discordGuilds).length > 0) {
        dockerfile = dockerfile.replace(/^ARG NEMOCLAW_DISCORD_GUILDS_B64=.*$/m, `ARG NEMOCLAW_DISCORD_GUILDS_B64=${encodeDockerJsonArg(discordGuilds)}`);
    }
...
function shouldRequireResponsesToolCalling(provider) {
    return (provider === "nvidia-prod" || provider === "gemini-api" || provider === "compatible-endpoint");
}
...
function probeOpenAiLikeEndpoint(endpointUrl, model, apiKey, options = {}) {
    const responsesProbe = options.requireResponsesToolCalling === true
        ? {
    ...
    const accountFailure = failures.find((failure) => isNvcfFunctionNotFoundForAccount(failure.message) ||
...
async function validateOpenAiLikeSelection(label, endpointUrl, model, credentialEnv = null, retryMessage = "Please choose a provider/model again.", helpUrl = null, options = {}) {
...
async function validateAnthropicSelectionWithRetryMessage(label, endpointUrl, model, credentialEnv, retryMessage = "Please choose a provider/model again.", helpUrl = null) {
...
const { shouldIncludeBuildContextPath, copyBuildContextDir, printSandboxCreateRecoveryHints } = buildContext;
...
function getNonInteractiveProvider() {
    ...
    if (!validProviders.has(normalized)) {
        ...
        console.error("  Valid values: build, openai, anthropic, anthropicCompatible, gemini, ollama, custom, compatible-endpoint, nim-local, vllm");
        process.exit(1);
    }
    return normalized;
}
...
async function preflight() {
    ...
    if (!isOpenshellInstalled()) {
        ...
        if (!currentVersion) {
            ...
            const needsUpgrade = parts[0] < minParts[0] ||
                (parts[0] === minParts[0] && parts[1] < minParts[1]) ||
                (parts[0] === minParts[0] && parts[1] === minParts[1] && parts[2] < minParts[2]);
            if (needsUpgrade) {
                console.log(`  openshell ${currentVersion} is below minimum required version. Upgrading...`);
                openshellInstall = installOpenshell();
                if (!openshellInstall.installed) {
    ...
    if (installedOpenshellVersion &&
        ...
        console.error("");
        console.error(`  ✗ openshell ${installedOpenshellVersion} is below the minimum required by this NemoClaw release.`);
    ...
    if (installedOpenshellVersion &&
        ...
        console.error("");
        console.error(`  ✗ openshell ${installedOpenshellVersion} is above the maximum supported by this NemoClaw release.`);
    ...
    if (gatewayReuseState === "missing") {
        ...
        const inspectResult = run(`docker inspect --type container --format '{{.State.Status}}' ${containerName} 2>/dev/null`, { ignoreError: true, suppressOutput: true });
        if (inspectResult.status === 0) {
            ...
            const postInspectResult = run(`docker inspect --type container ${containerName} 2>/dev/null`, {
                ignoreError: true,
                suppressOutput: true,
            });
            if (postInspectResult.status !== 0) {
                run(`docker volume ls -q --filter "name=openshell-cluster-${GATEWAY_NAME}" | grep . && docker volume ls -q --filter "name=openshell-cluster-${GATEWAY_NAME}" | xargs docker volume rm 2>/dev/null || true`, { ignoreError: true, suppressOutput: true });
                registry.clearAll();
    ...
    for (const { port, label } of requiredPorts) {
        ...
        if (!portCheck.ok) {
            ...
            if (port === DASHBOARD_PORT && portCheck.process === "ssh" && portCheck.pid) {
                ...
                const cmdline = runCapture(`ps -p ${portCheck.pid} -o args= 2>/dev/null`, { ignoreError: true }).trim();
    ...
    if (process.platform === "linux") {
        ...
        if (mem) {
            if (mem.totalMB < 12000) {
                ...
                if (!isNonInteractive()) {
                    const answer = await prompt("  Create a 4 GB swap file to prevent OOM during sandbox build? (requires sudo) [y/N]: ");
...
async function startGatewayWithOptions(_gpu, { exitOnFailure = true } = {}) {
    ...
    if (fs.existsSync(knownHostsPath)) {
        try {
            ...
            if (cleaned !== kh)
                fs.writeFileSync(knownHostsPath, cleaned);
        }
        catch {
    ...
    try {
        await pRetry(async () => {
            const startResult = await streamGatewayStart(openshellShellCommand(["gateway", "start", ...gwArgs]), {
                ...process.env,
                ...gatewayEnv,
            });
        ...
        if (exitOnFailure) {
            ...
            console.error("");
            try {
                const logs = redact(runCaptureOpenshell(["doctor", "logs", "--name", GATEWAY_NAME], {
                    ignoreError: true,
                }));
                if (logs) {
...
async function recoverGatewayRuntime() {
    ...
    if (startResult.status !== 0) {
        const diagnostic = compactText(redact(`${startResult.stderr || ""} ${startResult.stdout || ""}`));
...
async function promptValidatedSandboxName() {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        const nameAnswer = await promptOrDefault("  Sandbox name (lowercase, starts with letter, hyphens ok) [my-assistant]: ", "NEMOCLAW_SANDBOX_NAME", "my-assistant");
...
// ── Step 5: Sandbox ──────────────────────────────────────────────
// eslint-disable-next-line complexity
async function createSandbox(gpu, model, provider, preferredInferenceApi = null, sandboxNameOverride = null, webSearchConfig = null, enabledChannels = null, fromDockerfile = null, agent = null, dangerouslySkipPermissions = false) {
    ...
    const getMessagingToken = (envKey) => getCredential(envKey) || normalizeCredentialValue(process.env[envKey]) || null;
    ...
    if (conflictCheckChannels.length > 0) {
        ...
        if (conflicts.length > 0) {
            for (const { channel, sandbox } of conflicts) {
                console.log(`  ⚠ Sandbox '${sandbox}' already has ${channel} enabled. Bot tokens only allow one sandbox to poll — continuing will break both bridges.`);
            }
            if (isNonInteractive()) {
                console.error("  Aborting: resolve the messaging channel conflict above or run `nemoclaw <sandbox> destroy` on the other sandbox.");
                process.exit(1);
            }
    ...
    const enabledEnvKeys = enabledChannels != null
        ? new Set(MESSAGING_CHANNELS.filter((c) => enabledChannels.includes(c.name)).flatMap((c) => {
            const keys = [c.envKey];
            if (c.appTokenEnvKey)
                keys.push(c.appTokenEnvKey);
            ...
            try {
                ...
                if (backup.success) {
                    ...
                    for (const { envKey, token } of messagingTokenDefs) {
                        if (token)
                            abortHashes[envKey] = hashCredential(token);
                    }
    ...
    if (fromDockerfile) {
        ...
        if (agent.name === "hermes") {
            ...
            const phoenixEndpoint = getCredential(PHOENIX_COLLECTOR_ENDPOINT_ENV) ||
                process.env[PHOENIX_COLLECTOR_ENDPOINT_ENV];
            if (phoenixEndpoint) {
                dfContent = dfContent.replace(/^ARG PHOENIX_COLLECTOR_ENDPOINT=.*$/m, `ARG PHOENIX_COLLECTOR_ENDPOINT=${phoenixEndpoint}`);
                ...
                dfContent = dfContent.replace(/^(ARG BASE_IMAGE=ghcr\.io\/nvidia\/nemoclaw\/hermes-sandbox-base):latest$/m, `$1-nemo-flow:latest`);
                dfDirty = true;
            }
            const tokenManagerHost = getCredential(TOKEN_MANAGER_HOST_ENV) ||
                process.env[TOKEN_MANAGER_HOST_ENV];
            if (tokenManagerHost) {
                dfContent = dfContent.replace(/^ARG TOKEN_MANAGER_HOST=.*$/m, `ARG TOKEN_MANAGER_HOST=${tokenManagerHost}`);
                dfDirty = true;
            }
            if (dfDirty) {
    ...
    if (webSearchConfig && !getCredential(webSearch.BRAVE_API_KEY_ENV)) {
        ...
        console.error("  Re-run with BRAVE_API_KEY set, or disable Brave Search before recreating the sandbox.");
        process.exit(1);
    }
    const tokensByEnvKey = Object.fromEntries(messagingTokenDefs.map(({ envKey, token }) => [envKey, token]));
    const activeMessagingChannels = [
        ...new Set(messagingTokenDefs
            .filter(({ token }) => !!token)
            .map(({ envKey }) => {
            if (envKey === "DISCORD_BOT_TOKEN")
                return "discord";
            if (envKey === "SLACK_BOT_TOKEN")
                return "slack";
            ...
            if (envKey === "TELEGRAM_BOT_TOKEN")
                return "telegram";
            if (envKey === "OUTLOOK_CLIENT_ID")
                return "outlook";
            // OUTLOOK_TENANT_ID is a secondary provider credential — CLIENT_ID is primary.
            if (envKey === "OUTLOOK_TENANT_ID")
                return null;
            return null;
        })
            .filter(Boolean)),
    ...
    for (const ch of MESSAGING_CHANNELS) {
        if (enabledTokenEnvKeys.has(ch.envKey) &&
            ch.allowIdsMode === "dm" &&
            ch.userIdEnvKey &&
            ...
            const ids = process.env[ch.userIdEnvKey]
                ...
                .filter(Boolean);
            if (ids.length > 0)
                messagingAllowedIds[ch.name] = ids;
        }
    }
    const discordGuilds = {};
    ...
    if (resolved) {
        ...
        const localCheck = runCapture(["docker", "image", "inspect", `${SANDBOX_BASE_IMAGE}:${SANDBOX_BASE_TAG}`], { ignoreError: true });
        if (localCheck) {
    ...
    patchStagedDockerfile(stagedDockerfile, model, chatUiUrl, String(Date.now()), provider, preferredInferenceApi, webSearchConfig, activeMessagingChannels, messagingAllowedIds, discordGuilds, resolved ? resolved.ref : null);
    ...
    if (activeMessagingChannels.length > 0) {
        envArgs.push(formatEnvAssignment("NEMOCLAW_MESSAGING_CHANNELS_B64", encodeDockerJsonArg(activeMessagingChannels)));
    }
    ...
    if (agent?.name === "hermes") {
        ...
        const outlookTargetMailbox = getCredential("OUTLOOK_TARGET_MAILBOX") || process.env.OUTLOOK_TARGET_MAILBOX;
        if (outlookTargetMailbox) {
            envArgs.push(formatEnvAssignment("OUTLOOK_TARGET_MAILBOX", outlookTargetMailbox));
        }
        const outlookReplyTo = getCredential("OUTLOOK_REPLY_TO") || process.env.OUTLOOK_REPLY_TO;
        if (outlookReplyTo) {
            envArgs.push(formatEnvAssignment("OUTLOOK_REPLY_TO", outlookReplyTo));
        }
        const outlookAllowedSenders = getCredential("OUTLOOK_ALLOWED_SENDERS") || process.env.OUTLOOK_ALLOWED_SENDERS;
    ...
    if (process.env.GATEWAY_ALLOW_ALL_USERS) {
        envArgs.push(formatEnvAssignment("GATEWAY_ALLOW_ALL_USERS", process.env.GATEWAY_ALLOW_ALL_USERS));
    }
    if (process.env.NEMOCLAW_DECODE_PROXY_DEBUG) {
        envArgs.push(formatEnvAssignment("NEMOCLAW_DECODE_PROXY_DEBUG", process.env.NEMOCLAW_DECODE_PROXY_DEBUG));
    }
    ...
    for (let i = 0; i < 15; i++) {
        const readyMatch = runCapture(`openshell sandbox exec ${shellQuote(sandboxName)} curl -sf http://localhost:${DASHBOARD_PORT}/ 2>/dev/null || echo "no"`, { ignoreError: true });
    ...
    if (pendingStateRestore?.success) {
        ...
        const restore = sandboxState.restoreSandboxState(sandboxName, pendingStateRestore.manifest.backupPath);
        if (restore.success) {
            note(`  ✓ State restored (${restore.restoredDirs.length} directories)`);
        }
        else {
            console.error(`  Warning: partial restore. Manual recovery: ${pendingStateRestore.manifest.backupPath}`);
        }
    }
    // DNS proxy — run a forwarder in the sandbox pod so the isolated
    // sandbox namespace can resolve hostnames (fixes #626).
    console.log("  Setting up sandbox DNS proxy...");
    run(`bash "${path.join(SCRIPTS, "setup-dns-proxy.sh")}" ${shellQuote(GATEWAY_NAME)} ${shellQuote(sandboxName)} 2>&1 || true`, { ignoreError: true });
...
async function setupNim(gpu) {
    ...
    const ollamaRunning = !!runCapture(`curl -sf http://127.0.0.1:${OLLAMA_PORT}/api/tags 2>/dev/null`, {
        ignoreError: true,
    });
    ...
    if (hasOllama || ollamaRunning) {
        options.push({
            key: "ollama",
            label: `Local Ollama (localhost:${OLLAMA_PORT})${ollamaRunning ? " — running" : ""}` +
        ...
        selectionLoop: while (true) {
            ...
            if (isNonInteractive()) {
                ...
                const envProviderIdx = envProviderHint
                    ...
                    : -1;
                const defaultIdx = (envProviderIdx >= 0 ? envProviderIdx : options.findIndex((o) => o.key === "build")) + 1;
            ...
            if (REMOTE_PROVIDER_CONFIG[selected.key]) {
                ...
                if (selected.key === "custom") {
                    ...
                    const endpointInput = isNonInteractive()
                        ? _envUrl
                        : (await prompt(_envUrl
                            ? `  OpenAI-compatible base URL [${_envUrl}]: `
                            : "  OpenAI-compatible base URL (e.g., https://openrouter.ai): ")) || _envUrl;
                    ...
                    const endpointInput = isNonInteractive()
                        ...
                        : (await prompt(_envUrl
                            ...
                            : "  Anthropic-compatible base URL (e.g., https://proxy.example.com): ")) || _envUrl;
                    const navigation = getNavigationChoice(endpointInput);
                    if (navigation === "back") {
                        console.log("  Returning to provider selection.");
                        console.log("");
                ...
                if (selected.key === "build") {
                    ...
                    if (isNonInteractive()) {
                        if (!process.env.NVIDIA_API_KEY) {
                            console.error("  NVIDIA_API_KEY (or NEMOCLAW_PROVIDER_KEY) is required for NVIDIA Endpoints in non-interactive mode.");
                            process.exit(1);
                        }
                    ...
                    if (isNonInteractive()) {
                        if (!process.env[credentialEnv]) {
                            console.error(`  ${credentialEnv} (or NEMOCLAW_PROVIDER_KEY) is required for ${remoteConfig.label} in non-interactive mode.`);
                            process.exit(1);
                        }
                    }
                    else {
                    ...
                    if (selected.key === "openai" || selected.key === "gemini") {
                        modelValidator = (candidate) => validateOpenAiLikeModel(remoteConfig.label, endpointUrl, candidate, getCredential(credentialEnv));
                    }
                    else if (selected.key === "anthropic") {
                        modelValidator = (candidate) => validateAnthropicModel(endpointUrl || ANTHROPIC_ENDPOINT_URL, candidate, getCredential(credentialEnv));
                    }
                    while (true) {
                        if (isNonInteractive()) {
                            model = defaultModel;
                        ...
                        if (selected.key === "custom") {
                            const validation = await validateCustomOpenAiLikeSelection(remoteConfig.label, endpointUrl, model, credentialEnv, remoteConfig.helpUrl);
                            if (validation.ok) {
                                ...
                                if (explicitApi && explicitApi !== "openai-completions" && explicitApi !== "chat-completions") {
                                    ...
                                    if (validation.api !== "openai-completions") {
                                        console.log("  ℹ Using chat completions API (compatible endpoints may not support the Responses API developer role)");
                                    }
                                    preferredInferenceApi = "openai-completions";
                                }
                                break;
                            }
                            if (validation.retry === "credential" ||
                            ...
                            const validation = await validateCustomAnthropicSelection(remoteConfig.label, endpointUrl || ANTHROPIC_ENDPOINT_URL, model, credentialEnv, remoteConfig.helpUrl);
                            if (validation.ok) {
                                preferredInferenceApi = validation.api;
                                break;
                            }
                            ...
                            if (selected.key === "anthropic") {
                                const validation = await validateAnthropicSelectionWithRetryMessage(remoteConfig.label, endpointUrl || ANTHROPIC_ENDPOINT_URL, model, credentialEnv, retryMessage, remoteConfig.helpUrl);
                                if (validation.ok) {
                                    preferredInferenceApi = validation.api;
                                    break;
                                }
                                ...
                                const validation = await validateOpenAiLikeSelection(remoteConfig.label, endpointUrl, model, credentialEnv, retryMessage, remoteConfig.helpUrl, {
                ...
                if (selected.key === "build") {
                    while (true) {
                        const validation = await validateOpenAiLikeSelection(remoteConfig.label, endpointUrl, model, credentialEnv, "Please choose a provider/model again.", remoteConfig.helpUrl, {
                            requireResponsesToolCalling: shouldRequireResponsesToolCalling(provider),
                            skipResponsesProbe: shouldSkipResponsesProbe(provider),
                        });
                        if (validation.ok) {
                ...
                if (models.length === 0) {
                    ...
                    if (!nim.waitForNimHealth()) {
                        ...
                        const validation = await validateOpenAiLikeSelection("Local NVIDIA NIM", endpointUrl, model, credentialEnv);
                        if (validation.retry === "selection" ||
                ...
                while (true) {
                    ...
                    const validation = await validateOpenAiLikeSelection("Local Ollama", getLocalProviderValidationBaseUrl(provider), model, null, "Choose a different Ollama model or select Other.");
                    ...
                    if (validation.api !== "openai-completions") {
                        console.log("  ℹ Using chat completions API (Ollama tool calls require /v1/chat/completions)");
                    }
                    preferredInferenceApi = "openai-completions";
                    break;
                }
                ...
                const vllmModelsRaw = runCapture(`curl -sf http://127.0.0.1:${VLLM_PORT}/v1/models 2>/dev/null`, {
                    ignoreError: true,
                });
                try {
                    ...
                    console.error(`  Could not query vLLM models endpoint. Is vLLM running on localhost:${VLLM_PORT}?`);
                    process.exit(1);
                }
                const validation = await validateOpenAiLikeSelection("Local vLLM", getLocalProviderValidationBaseUrl(provider), model, credentialEnv);
                if (validation.retry === "selection" ||
                    validation.retry === "back" ||
                    validation.retry === "model") {
                    continue selectionLoop;
...
async function setupInference(sandboxName, model, provider, endpointUrl = null, credentialEnv = null) {
    ...
    if (provider === "nvidia-prod" ||
        ...
        while (true) {
            ...
            const message = compactText(redact(`${applyResult.stderr || ""} ${applyResult.stdout || ""}`)) ||
...
const MESSAGING_CHANNELS = [
    ...
    },
    {
        ...
        serverIdHelp: "Enable Developer Mode in Discord, then right-click your server and copy the Server ID.",
        serverIdLabel: "Discord Server ID (for guild workspace access)",
        requireMentionEnvKey: "DISCORD_REQUIRE_MENTION",
        requireMentionHelp: "Choose whether the bot should reply only when @mentioned or to all messages in this server.",
        userIdEnvKey: "DISCORD_USER_ID",
        userIdHelp: "Optional: enable Developer Mode in Discord, then right-click your user/avatar and copy the User ID. Leave blank to allow any member of the configured server to message the bot.",
    ...
    },
    {
        ...
        userIdHelp: "Open Slack, click your name/avatar → Profile → ⋮ → Copy member ID (e.g. U0887Q5UVV4).",
...
async function setupMessagingChannels() {
    ...
    const enabled = new Set(MESSAGING_CHANNELS.filter((c) => getMessagingToken(c.envKey)).map((c) => c.name));
    ...
    for (const name of selected) {
        ...
        if (ch.appTokenEnvKey) {
            ...
            if (existingAppToken) {
                ...
                const appToken = normalizeCredentialValue(await prompt(`  ${ch.appTokenLabel}: `, { secret: true }));
                if (appToken) {
        ...
        if (ch.userIdEnvKey && (!ch.serverIdEnvKey || process.env[ch.serverIdEnvKey])) {
            ...
            if (existingIds) {
                ...
                if (userId) {
                    ...
                    const skippedReason = ch.allowIdsMode === "guild"
        ...
        if (ch.name === "outlook") {
            for (const [envKey, label, isSecret] of [
                ...
                if (existing) {
                    ...
                    const val = normalizeCredentialValue(await prompt(`  ${label}: `, { secret: isSecret }));
                    if (val) {
...
function isChannelFullyConfigured(channel) {
    ...
    if (!def)
        return false;
    const keysToCheck = def.requiredEnvKeys ?? [def.envKey];
...
function getSuggestedPolicyPresets({ enabledChannels = null, webSearchConfig = null } = {}) {
    ...
    const maybeSuggestMessagingPreset = (channel, envKey) => {
        if (!isChannelFullyConfigured(channel))
            return;
...
async function setupOpenclaw(sandboxName, model, provider) {
    ...
    if (selectionConfig) {
        ...
        try {
            run(`${openshellShellCommand(["sandbox", "connect", sandboxName])} < ${shellQuote(scriptFile)}`, { stdio: ["ignore", "ignore", "inherit"] });
        }
        finally {
...
async function selectPolicyTier() {
    ...
    if (isNonInteractive()) {
        ...
        if (!tiers.getTier(name)) {
            console.error(`  Unknown policy tier: ${name}. Valid: ${allTiers.map((t) => t.name).join(", ")}`);
            process.exit(1);
        }
    ...
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        ...
        console.log("");
        const answer = await prompt(`  Select tier [1-${allTiers.length}] (default: ${allTiers.indexOf(defaultTier) + 1} ${defaultTier.name}): `);
...
async function selectTierPresetsAndAccess(tierName, allPresets, extraSelected = []) {
    ...
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        ...
        console.log("");
        const rawInclude = await prompt("  Include presets (comma-separated names, Enter to keep defaults): ");
        if (rawInclude.trim()) {
...
async function setupPoliciesWithSelection(sandboxName, options = {}) {
    ...
    if (webSearchConfig && !suggestions.includes("brave"))
        suggestions.push("brave");
    if (isNonInteractive()) {
        const policyMode = (process.env.NEMOCLAW_POLICY_MODE || "suggested").trim().toLowerCase();
        chosen = suggestions;
        ...
        if (policyMode === "custom" || policyMode === "list") {
            ...
            if (envPresets.length > 0)
                chosen = [...new Set([...suggestions, ...envPresets])];
        }
        else {
            console.error(`  Unsupported NEMOCLAW_POLICY_MODE: ${policyMode}`);
            console.error("  Valid values: suggested, custom, skip");
...
function fetchGatewayAuthTokenFromSandbox(sandboxName) {
    ...
    try {
        ...
        const result = runOpenshell(["sandbox", "download", sandboxName, "/sandbox/.openclaw/openclaw.json", destDir], { ignoreError: true, stdio: ["ignore", "ignore", "ignore"] });
        if (result.status !== 0)
            return null;
...
function getDashboardForwardPort(chatUiUrl = process.env.CHAT_UI_URL || `http://127.0.0.1:${CONTROL_UI_PORT}`) {
...
function getDashboardForwardTarget(chatUiUrl = process.env.CHAT_UI_URL || `http://127.0.0.1:${CONTROL_UI_PORT}`, options = {}) {
...
function getDashboardForwardStartCommand(sandboxName, options = {}) {
    ...
    return `${openshellShellCommand(["forward", "start", "--background", forwardTarget, sandboxName], options)}`;
}
...
function getDashboardAccessInfo(sandboxName, options = {}) {
    ...
    if (wslHostAddress) {
        const wslUrl = buildAuthenticatedDashboardUrl(`http://${wslHostAddress}:${dashboardPort}/`, token);
...
function getDashboardGuidanceLines(dashboardAccess = [], options = {}) {
    const dashboardPort = getDashboardForwardPort(options.chatUiUrl || process.env.CHAT_UI_URL || `http://127.0.0.1:${CONTROL_UI_PORT}`);
    const guidance = [`Port ${dashboardPort} must be forwarded before opening these URLs.`];
    if (isWsl(options)) {
        guidance.push("WSL detected: if localhost fails in Windows, use the WSL host IP shown by `hostname -I`.");
    }
...
function printDashboard(sandboxName, model, provider, nimContainer = null, agent = null) {
    ...
    if (provider === "nvidia-prod" || provider === "nvidia-nim")
    ...
    else if (provider === "compatible-endpoint")
    ...
    if (agent) {
        agentOnboard.printDashboardUi(sandboxName, token, agent, {
            ...
            buildControlUiUrls: (tokenValue, port) => {
                ...
                if (wslHostAddress) {
                    const wslUrl = buildAuthenticatedDashboardUrl(`http://${wslHostAddress}:${port}/`, tokenValue);
...
function startRecordedStep(stepName, updates = {}) {
    ...
    if (Object.keys(updates).length > 0) {
        onboardSession.updateSession((session) => {
            if (typeof updates.sandboxName === "string")
                session.sandboxName = updates.sandboxName;
...
async function onboard(opts = {}) {
    ...
    const dangerouslySkipPermissions = opts.dangerouslySkipPermissions || process.env.NEMOCLAW_DANGEROUSLY_SKIP_PERMISSIONS === "1";
    if (dangerouslySkipPermissions) {
        console.error("");
    ...
    const requestedFromDockerfile = opts.fromDockerfile ||
    ...
    const lockResult = onboardSession.acquireOnboardLock(`nemoclaw onboard${resume ? " --resume" : ""}${isNonInteractive() ? " --non-interactive" : ""}${requestedFromDockerfile ? ` --from ${requestedFromDockerfile}` : ""}`);
    ...
    try {
        ...
        if (resume) {
            ...
            if (resumeConflicts.length > 0) {
                for (const conflict of resumeConflicts) {
                    if (conflict.field === "sandbox") {
                        console.error(`  Resumable state belongs to sandbox '${conflict.recorded}', not '${conflict.requested}'.`);
                    }
                    else if (conflict.field === "agent") {
                        console.error(`  Session was started with agent '${conflict.recorded}', not '${conflict.requested}'.`);
                    }
                        ...
                        console.error(`  Resumable state recorded ${conflict.field} '${conflict.recorded}', not '${conflict.requested}'.`);
                    }
                }
            ...
            session = onboardSession.saveSession(onboardSession.createSession({
        ...
        while (true) {
            const resumeProviderSelection = !forceProviderSelection &&
                resume &&
            ...
            const resumeInference = !forceProviderSelection &&
                resume &&
                typeof provider === "string" &&
                typeof model === "string" &&
            ...
            const inferenceResult = await setupInference(sandboxName, model, provider, endpointUrl, credentialEnv);
        ...
        if (resumeSandbox) {
            ...
            await configureOutlookTokenManager(agent, selectedMessagingChannels.includes("outlook"));
            sandboxName = await createSandbox(gpu, model, provider, preferredInferenceApi, sandboxName, webSearchConfig, selectedMessagingChannels, fromDockerfile, agent, dangerouslySkipPermissions);
        ...
        if (dangerouslySkipPermissions) {
            ...
            const resumePolicies = !RECREATE_SANDBOX &&
