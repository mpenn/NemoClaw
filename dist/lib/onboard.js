...
async function createSandbox(gpu, model, provider, preferredInferenceApi = null, sandboxNameOverride = null, webSearchConfig = null, enabledChannels = null, fromDockerfile = null, agent = null, dangerouslySkipPermissions = false) {
    ...
    const enabledEnvKeys = enabledChannels != null
        ? new Set(MESSAGING_CHANNELS.filter((c) => enabledChannels.includes(c.name)).flatMap((c) => {
            ...
            if (c.name === "outlook")
                keys.push("OUTLOOK_CLIENT_SECRET", "OUTLOOK_TENANT_ID", "OUTLOOK_BOT_MAILBOX", "OUTLOOK_USER_MAILBOX", "OUTLOOK_BASIC_AUTH");
            return keys;
        }))
        : null;
    ...
    if (fromDockerfile) {
        ...
        if (agent.name === "hermes") {
            ...
            if (phoenixEndpoint) {
                ...
                fs.writeFileSync(stagedDockerfile, dfContent.replace(/^ARG PHOENIX_COLLECTOR_ENDPOINT=.*$/m, `ARG PHOENIX_COLLECTOR_ENDPOINT=${phoenixEndpoint}`));
            }
        }
    }
    else {
    ...
    const activeMessagingChannels = [
        ...new Set(messagingTokenDefs
            ...
            if (envKey === "OUTLOOK_CLIENT_SECRET" ||
                envKey === "OUTLOOK_TENANT_ID" ||
                envKey === "OUTLOOK_BOT_MAILBOX" ||
                envKey === "OUTLOOK_USER_MAILBOX" ||
                envKey === "OUTLOOK_BASIC_AUTH")
...
async function setupMessagingChannels() {
    ...
    for (const name of selected) {
        ...
        if (ch.name === "outlook") {
            ...
            const clientSecret = process.env.OUTLOOK_CLIENT_SECRET || getMessagingToken("OUTLOOK_CLIENT_SECRET") || "";
